...
#include "TlHelp32.h" // for procces list
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		switch (wmId)
		{
		case IDM_PROCESSLIST:
			DialogBox(hInst, MAKEINTRESOURCE(IDD_PROCESSLISTBOX), hWnd, ProcessList);
			break;
		case IDM_EXIT:
			DestroyWindow(hWnd);
			break;
...
DWORD WINAPI ThreadProc(LPVOID lpParam)
{
	...
	for (;;)
	{
		...
		SetDlgItemText(Thread.handleDialog, ID_MEM2, mem_char);
